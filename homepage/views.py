...
def login_view(request):
    ...
    if request.method == 'POST':
        text1 = "Welcome to Mail Sahayak, your very own Voice based Email service. Login with your email account in order to continue. "
        ...
        while (flag):
            ...
            if addr != 'N':
                ...
                i = i + str(1)
                say = speechtotext(3)
                if say == 'yes' or say == 'yas' or "yes" in say or "yas" in say:
                    flag = False
            else:
        ...
        print(addr)
        addr = addr.strip()
        print("after strip =", addr)
        addr = addr.replace(' ', '')
        print("after removing spaces =", addr)
        addr = addr.lower()
        print("after lower =", addr)
...
def options_view(request):
    ...
    if request.method == 'POST':
        flag = True
        texttospeech(
            "You are logged into your account. How would you like to proceed ?", file + i)
        i = i + str(1)
        while (flag):
            texttospeech(
                "To compose an email say compose. To open Inbox folder say Inbox. To Logout say Logout. Do you want me to repeat?", file + i)
            i = i + str(1)
            say = speechtotext(3)
            if say == 'No' or say == 'no':
                flag = False
